"""Catalogue des rôles — Le Village Maudit."""
...
def role_camp(role_id):
    r = ROLES.get(role_id)
    return r["camp"] if r else None


def has_special_roles(role_pool):
    return any("special" in ROLES.get(rid, {}) for rid in role_pool)


def suggest_roles(n):
    if n < 4:
        return []
    nb_vampires = max(1, round(n / 3))
    picks = ["vampire"] * nb_vampires
    extras = []
    if n >= 6:
        extras.append("medium")
    if n >= 7:
        extras.append("alchimiste")
    if n >= 8:
        extras.append("chasseur-vampires")
    if n >= 9:
        extras.append("lieur-sang")
    if n >= 11:
        extras.append("fillette-curieuse")
    if n >= 13:
        extras.append("roublard")
    if n >= 15:
        extras.append("veilleur")
    picks += extras
    while len(picks) < n:
        picks.append("paysan")
    return picks[:n]
